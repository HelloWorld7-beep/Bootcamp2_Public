  ...
  */
    ...
    */
      async function retrieveAllListings() {
          //ADD CODE HERE
          const retrieveAll = await Listing.findAll();
   ...
   */
    ...
    */
      async function removeCable() {
         //ADD CODE HERE
          const cable = await Listing.findOne({
              where: {
                  code: "CABL"
              }
          });
    ...
    */
    async function addDSIT() {
       //ADD CODE HERE
        const addDSIT = await Listing.create({
            code: "DSIT",
    ...
    */
    ...
    async function updatePhelpsLab() {
       //ADD CODE HERE
        const pLab = await Listing.findOne({
            where: {
                code: "PHL"
            }
        });
        ...
        if (pLab)
        {
            pLab.address = "1953 Museum Rd, Gainesville, FL 32603";
            await pLab.save();
            console.log(pLab);
        }
...
async function runFunctionsSequentially() {
    //Wrapped these in an async function since I was getting errors for too many DB connections.
